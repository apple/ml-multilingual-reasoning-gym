...
@dataclass
class ComplexArithmeticConfig(DatasetConfig):
    min_real: int = -10
    max_real: int = 10
    min_imag: int = -10
    max_imag: int = 10
...
class ComplexArithmeticDataset(MultilingualProceduralDataset):
    ...
    def _generate_item(self, idx: int, language: str) -> dict[str, Any]:
        """Generate a complex arithmetic problem for a given index and language."""
        ...
        # Get the question template for this operation
        question = self._get_translation(f"operation_{op}", language, 
