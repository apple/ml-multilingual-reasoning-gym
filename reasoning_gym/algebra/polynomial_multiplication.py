...
@dataclass
class PolynomialMultiplicationConfig(DatasetConfig):
    """
...
class PolynomialMultiplicationDataset(MultilingualProceduralDataset):
    """
    ...
    def _generate_item(self, idx: int, language: str) -> dict[str, Any]:
        """
        Generate a single polynomial multiplication item for the specified language.

        Args:
            idx: The index of the item to generate
            language: The language code for the item

        Returns:
            A dict with:
                - question: str (e.g. "Multiply polynomials: (8x^3 + x + 2)*(x - 3)")
                - answer: str (Product, e.g. "8x^4 - 24x^3 + x^2 - x - 6")
                - metadata: dict with details (polynomial_expr, result, variables, language)
        """
        ...
        # Generate the question using translation templates
        prompt_template_key = f"prompt_template_{rng.randint(1, 2)}"
        question_text = self._get_translation(prompt_template_key, language, polynomial_expr=polynomial_expr)
        instruction = self._get_translation("instruction", language)
        ...
        return {
            ...
            "metadata": {
                ...
                "variables": sorted([str(x) for x in product.free_symbols]),
                "language": language,
                "difficulty": {
    ...
    def _get_monomials(self, rng: random.Random) -> str:
        ...
        monomials = sorted(list(itermonomials(sym, self.config.max_degree, self.config.min_degree)), key=str)
        return monomials
