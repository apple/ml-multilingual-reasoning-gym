...
@dataclass
class SimpleIntegrationConfig(DatasetConfig):
    min_terms: int = 2
    max_terms: int = 5
    min_degree: int = 1
    max_degree: int = 10
...
class SimpleIntegrationDataset(MultilingualProceduralDataset):
    ...
    def _generate_item(self, idx: int, language: str) -> dict[str, Any]:
        """Generate a single item for the dataset.
        ...
        # Choose a random prompt template and format the question
        prompt_key = rng.choice(["prompt_template_1", "prompt_template_2", "prompt_template_3"])
        question = self._get_translation(prompt_key, language, integrand=derivative)
        
        # Add the instruction
        instruction = self._get_translation("calculation_instructions", language)
