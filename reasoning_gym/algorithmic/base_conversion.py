...
@dataclass
class BaseConversionConfig(DatasetConfig):
...
class BaseConversionDataset(MultilingualProceduralDataset):
    """Generates base conversion tasks"""

    def __init__(self, config: BaseConversionConfig):
        super().__init__(config=config, seed=config.seed, size=config.size)

    def _format_base_name(self, base: int, language: str) -> str:
        """Get human-readable name for common bases"""
        if base == 2:
            return self._get_translation("binary", language)
        elif base == 16:
            return self._get_translation("hexadecimal", language)
        else:
            return self._get_translation("base_template", language, base=base)
    ...
    def _generate_item(self, idx: int, language: str) -> dict[str, Any]:
        """Generate a single base conversion task for a specific language"""
        ...
        source_name = self._format_base_name(source_base, language)
        target_name = self._format_base_name(target_base, language)
