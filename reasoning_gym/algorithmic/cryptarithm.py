...
@dataclass
class CryptarithmConfig(DatasetConfig):
...
class CryptarithmDataset(MultilingualProceduralDataset):
    """
    ...
    def _generate_item(self, idx: int, language: str) -> dict[str, Any]:
        """Generate a single multilingual cryptarithm puzzle item."""
        rng = Random(self.seed + idx)
        return self._create_single_puzzle(rng, idx, language)

    def _create_single_puzzle(self, rng: Random, idx: int, language: str) -> dict:
        """
        ...
        question_str = (
            self._get_translation("intro", language) + "\n\n"
            f"{puzzle_text}\n\n"
            + self._get_translation("unique_digit_rule", language)
            + (
                self._get_translation("leading_zero_allowed", language)
                if self.config.allow_leading_zero
                else self._get_translation("leading_zero_not_allowed", language)
            )
            + self._get_translation("output_format", language)
        )
