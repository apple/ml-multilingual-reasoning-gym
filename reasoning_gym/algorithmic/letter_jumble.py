...
@dataclass
class LetterJumbleConfig(DatasetConfig):
...
class LetterJumbleDataset(MultilingualProceduralDataset):
    ...
    def _generate_item(self, idx: int, language: str) -> dict[str, Any]:
        """Generate a single word jumbling task for a specific language.
        
        Args:
            idx (int): The item index.
            language (str): The target language for the task.
            
        Returns:
            dict[str, Any]: A dictionary with 'question', 'answer', and 'metadata' keys.
        """
        ...
        # Use translation template for the question
        question = self._get_translation("question_template", language, words=" ".join(scrambled_words))
