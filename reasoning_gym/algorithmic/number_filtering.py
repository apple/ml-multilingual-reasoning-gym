...
@dataclass
class NumberFilteringConfig(DatasetConfig):
...
class NumberFilteringDataset(MultilingualProceduralDataset):
    ...
    def _generate_item(self, idx: int, language: str) -> dict[str, Any]:
        """Generate a single number filtering task for a specific language"""
        ...
        # Generate localized question
        question = self._get_translation(
            "question_template",
            language,
            action=self._get_translation(f"action_{keep_remove}", language),
            comparison=self._get_translation(f"comparison_{larger_smaller}", language),
            filter_value=filter_str,
            numbers_list=str(str_numbers)
        )
