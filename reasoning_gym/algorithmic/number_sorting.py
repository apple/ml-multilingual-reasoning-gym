...
@dataclass
class NumberSortingConfig(DatasetConfig):
    ...
    language_weights: Optional[list[float]] = None  # Add optional language weights
...
class NumberSortingDataset(MultilingualProceduralDataset):
    ...
    def _generate_item(self, idx: int, language: str) -> dict[str, Any]:
        """Generate a single sorting task for a specific language.
        ...
        direction_text = self._get_translation(f"{direction}_direction", language)
        numbers_list = ', '.join(number_strs)
        question_main = self._get_translation("sort_question", language, 
                                            direction=direction_text, 
                                            numbers=numbers_list)
        instruction = self._get_translation("answer_format_instruction", language)
        question = question_main + instruction
