...
@dataclass
class WordSortingConfig(DatasetConfig):
...
class WordSortingDataset(MultilingualProceduralDataset):
    ...
    def _generate_item(self, idx: int, language: str) -> dict[str, Any]:
        ...
        direction_key = "ascending" if is_ascending else "descending"
        answer = asc_words if is_ascending else desc_words

        # Get translated direction and format question
        direction = self._get_translation(direction_key, language)
        question = self._get_translation("question_template", language, 
                                       direction=direction, 
                                       words=", ".join(transformed_words))
