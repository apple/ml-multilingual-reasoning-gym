...
@dataclass
class BasicArithmeticDatasetConfig(DatasetConfig):
...
class BasicArithmeticDataset(MultilingualProceduralDataset):
    ...
    def _generate_item(self, idx: int, language: str) -> dict[str, Any]:
        """Generate a single arithmetic task for a specific language
        ...
        question = self._format_question(rng, expression, language)
    ...
    def _format_question(self, rng: Random, expression: str, language: str) -> str:
        """Format the the question with the arithmetic expression"""

        if self.config.format_style == "simple":
            return self._get_translation("simple_question", language, expression=expression)
        else:
            templates = ["question_what", "question_solve", "question_compute", "question_evaluate"]
            template_key = rng.choice(templates)
            return self._get_translation(template_key, language, expression=expression)
