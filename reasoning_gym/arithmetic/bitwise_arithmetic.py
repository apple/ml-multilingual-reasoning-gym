...
@dataclass
class BitwiseArithmeticConfig(DatasetConfig):
...
class BitwiseArithmeticDataset(MultilingualProceduralDataset):
    ...
    def _generate_item(self, idx: int, language: str) -> dict[str, Any]:
        """
        Generate a single arithmetic task with language support.
        ...
        problem_str = self._get_translation("question_template", language, problem=problem)
