...
@dataclass
class DecimalChainSumConfig(DatasetConfig):
...
class DecimalChainSumDataset(MultilingualProceduralDataset):
    ...
    def _generate_item(self, idx: int, language: str) -> dict[str, Any]:
        """Generate a single decimal chain sum task for a specific language
        ...
        question = self._get_translation("question_template", language, expression=expression)
