...
def generate_puzzle(num_dice, max_dice_size, rng):
    ...
    # Return the puzzle with individual dice for localized formatting.
    return {"dice": dice, "target": target, "num": num, "den": den, "probability": probability}

@dataclass
class DiceConfig(DatasetConfig):
...
class DiceDataset(MultilingualProceduralDataset):
    ...
    def _generate_item(self, idx: int, language: str) -> dict:
        ...
        # Get localized dice separator
        dice_separator = self._get_translation("dice_separator", language)
        
        # Format dice string with localized separator
        dice_str = dice_separator.join(f"1d{s}" for s in puzzle['dice'])
        
        puzzle_str = self._get_translation(
            "puzzle_question", 
            language, 
            dice_str=dice_str, 
