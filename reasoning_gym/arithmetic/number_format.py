...
@dataclass
class NumberFormatConfig(DatasetConfig):
...
class NumberFormatDataset(MultilingualProceduralDataset):
    ...
    def _generate_item(self, idx: int, language: str) -> dict[str, Any]:
        """Generate a single Number Format question for the specified language"""
        ...
        question = self._get_translation(
            "question_template",
            language,
            size=self._get_translation(f"size_{size}", language),
            numbers=" ".join(formatted_candidates)
        )
