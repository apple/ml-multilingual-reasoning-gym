...
class DefaultCurriculumContext(CurriculumContext):
    def __init__(self, mode: RangeAttributeMode = RangeAttributeMode.INCLUSIVE, k: int | None = 3):
...
class BaseCurriculum:
    ...
    def set_global_level_percentile(self, percentile: float) -> None:
        """
        Set all attributes to a level based on a percentile of the maximum available level.

        Args:
            percentile: A float between 0.0 and 1.0 representing the percentile of the maximum
                       available level across all attributes (e.g., 0.5 for 50th percentile)
        ...
        # Calculate the maximum available level across all attributes
        max_available_level = max(len(attr.levels) - 1 for attr in self._attributes.values())

        # Calculate target level based on percentile
        target_level = round(percentile * max_available_level)

        # Use existing set_global_level logic
        self.set_global_level(target_level)
    ...
    def get_global_level(self) -> Optional[int]:
        ...
        for attr_name in self._attributes:
            attr = self.get_attribute(attr_name)
            if isinstance(attr, RangeAttributeDefinition):
                lo, hi = DefaultCurriculumContext().get_range_attr_value(self, attr)
                attr_dict[attr.lower_field_name] = lo
                attr_dict[attr.upper_field_name] = hi
            elif isinstance(attr, ScalarAttributeDefinition):
