...
class CurriculumExperiment(Experiment):
    def __init__(
        ...
        for dataset_name, attr_config in config.curricula.items():
            ...
            # very important! set seed here s.t. all ranks have same seed
            # also very important! set size here b/c otherwise we can get index out of bound error in list_functions
            dataset_config = curriculum.generate_configuration(context=context, defaults={"seed": seed, "size": size})
