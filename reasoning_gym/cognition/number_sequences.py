...
@dataclass
class NumberSequenceConfig(DatasetConfig):
...
class NumberSequenceDataset(MultilingualProceduralDataset):
    ...
    def _generate_item(self, idx: int, language: str) -> dict[str, Any]:
        """Generate a sequence task with a newly generated pattern for the specified language"""
        ...
        for _ in range(max_attempts):
            ...
            try:
                for i in range(num_terms):  # Generate terms
                    ...
                    if abs(next_term) > self.config.max_value:
                        break
        ...
        question_mark = self._get_translation("question_mark", language)
        question = list_separator.join(map(str, visible_terms + [question_mark]))
