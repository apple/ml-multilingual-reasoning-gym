...
@dataclass
class RubiksCubeConfig(DatasetConfig):
...
class RubiksCubeDataset(MultilingualProceduralDataset):
    ...
    def _generate_item(self, idx: int, language: str) -> dict[str, Any]:
        """Generate a single RubiksCube task for a given language
        ...
        # Choose a prompt template using random selection
        template_options = ["prompt_template_1", "prompt_template_2"]
        template_key = rng.choice(template_options)
        question = self._get_translation(template_key, language, 
                                       cube_size=self.config.cube_size, 
                                       cube_render=cube_render)
