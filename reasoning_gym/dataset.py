...
class ProceduralDataset(ABC, Sized, Iterable[dict[str, Any]]):
    ...
    def task_name(self) -> str:
        """Get the task name for translations (derived from class name or module)"""
        # Try to get from module name first
        module_name = self.__class__.__module__
        if "." in module_name:
            return module_name.split(".")[-1]
        # Fallback to class name (remove "Dataset" suffix if present)
