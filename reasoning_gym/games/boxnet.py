...
@dataclass
class BoxnetConfig(DatasetConfig):
...
class BoxnetDataset(MultilingualProceduralDataset):
    def __init__(self, config: BoxnetConfig):
        super().__init__(config=config, seed=config.seed, size=config.size)

    def _generate_item(self, idx: int, language: str) -> dict[str, Any]:
        """Generate a single Boxnet task instance.
        
        Args:
            idx: The index of the instance to generate
            language: The language code for the instance
        ...
        state_update_prompt = self.state_update_func(row_num, column_num, pg_dict_initial, language)
        
        # Use translation for the main prompt
        boxnet_prompt = self._get_translation("boxnet_prompt", language)
        current_state_prompt = self._get_translation("current_state_intro", language)
        question = boxnet_prompt + "\n\n" + current_state_prompt + state_update_prompt + "\n"
    ...
    def state_update_func(self, pg_row_num, pg_column_num, pg_dict, language: str):
        state_update_prompt = ""
        for i in range(pg_row_num):
            for j in range(pg_column_num):
                ...
                agent_status = self._get_translation("agent_status", language, 
                                                   agent_x=i+0.5, agent_y=j+0.5, 
                                                   square_x=i+0.5, square_y=j+0.5,
                                                   items=square_item_list)
                state_update_prompt += agent_status + " "
