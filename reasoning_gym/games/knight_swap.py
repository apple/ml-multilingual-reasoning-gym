...
@dataclass
class KnightSwapConfig(DatasetConfig):
...
class KnightSwapDataset(MultilingualProceduralDataset):
    ...
    def _format_moves(self, moves: list[tuple[str, str, str]], language: str) -> str:
        """Format the solution moves as a string."""
        if not moves:
            return self._get_translation("impossible_answer", language)
    ...
    def _generate_item(self, idx: int, language: str) -> dict[str, Any]:
        """Generate a single Knight Swap puzzle in the specified language."""
        ...
        while board_attempts < self.config.max_attempts:
            try:
                ...
                positions = sorted(board.keys())  # Sort to ensure deterministic sampling
                ...
                for _ in range(50):  # Reduced attempts per board since we try multiple boards
                    ...
                    for start_turn in ["w", "B"]:
                        ...
                        if (make_impossible and solution is None) or (
                            ...
                            solution_str = self._format_moves(solution, language) if solution else self._get_translation("impossible_answer", language)
                            ...
                            # Create question using translations
                            impossible_answer = self._get_translation("impossible_answer", language)
                            question = self._get_translation("prompt_template", language, 
                                                           board_str=board_str, 
                                                           start_turn=start_turn,
                                                           impossible_answer=impossible_answer)
    ...
    def score_answer(self, answer: Optional[str], entry: dict) -> float:
        ...
        - The localized impossible answer (from translation) if the puzzle is impossible
        - A JSON list of moves in format ["color,start,end", ...] where color is 'w' or 'B'

        Returns:
        - 1.0 for correct answer (either impossible answer for impossible puzzles or valid solution of optimal length)
        ...
        # Get the localized impossible answer
        language = entry["metadata"]["language"]
        impossible_answer = self._get_translation("impossible_answer", language)

        # Handle impossible puzzles
        if not entry["metadata"]["is_possible"]:
            return 1.0 if answer.lower() == impossible_answer.lower() else 0.0

        # Handle impossible answer for possible puzzles
        if answer.lower() == impossible_answer.lower():
            return 0.0
