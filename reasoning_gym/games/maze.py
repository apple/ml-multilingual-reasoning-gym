...
@dataclass
class MazeConfig(DatasetConfig):
    """
...
class MazeDataset(MultilingualProceduralDataset):
    """
    ...
    def _generate_item(self, idx: int, language: str) -> dict[str, Any]:
        ...
        Returns:
            ...
            - "metadata": includes the grid, BFS distance, language, etc.
        """
        ...
        for _attempt in range(self.num_retries):
            ...
            if dist is not None and self.config.min_dist <= dist <= self.config.max_dist:
                ...
                question_str = self._get_translation("prompt_template", language,
                                                   start_char=self.start_char,
                                                   goal_char=self.goal_char,
                                                   maze_str=self._maze_to_str(maze_grid),
                                                   wall_char=self.wall_char,
                                                   path_char=self.path_char)
