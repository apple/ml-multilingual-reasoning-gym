...
@dataclass
class Puzzle24Config(DatasetConfig):
    operators: tuple = ("+", "-", "*", "/")
    min_value: int = 1
    max_value: int = 10
...
class Puzzle24Dataset(MultilingualProceduralDataset):
    ...
    def _generate_item(self, idx: int, language: str) -> dict[str, Any]:
        ...
        # Format strings with localized separator
        numbers_str = separator.join(map(str, numbers))
        operators_str = separator.join(self.config.operators)
