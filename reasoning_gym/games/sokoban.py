...
@dataclass
class SokobanConfig(DatasetConfig):
...
class SokobanDataset(MultilingualProceduralDataset):
    ...
    def _generate_item(self, idx: int, language: str) -> dict[str, Any]:
        """Generate a single Sokoban task for the given language

        Args:
            idx: The item index
            language: The language code for the task
        ...
        # Convert LURD solution to arrows
        lurd_to_arrow = {'L': '←', 'D': '↓', 'U': '↑', 'R': '→'}
        arrow_solution = ''.join(lurd_to_arrow.get(move, move) for move in solution)

        return {
            "question": self._get_translation("instruction_text", language) + gamestr,
            "answer": arrow_solution,
            "metadata": {
    ...
    def score_answer(self, answer: Optional[str], entry: dict[str, Any]) -> float:
        ...
        try:
            ...
            # Only accept pure arrow format
            arrow_to_lurd = {'←': 'L', '↓': 'D', '↑': 'U', '→': 'R'}
            valid_arrows = set(arrow_to_lurd.keys())
            
            # Check if answer contains only valid arrows
            if not all(move in valid_arrows for move in answer):
                return 0.0
            
            for move in answer:
                lurd_move = arrow_to_lurd[move]
                game.player.update(key=lurd_move)
