...
@dataclass
class HanoiConfig(DatasetConfig):
    """
    ...
    - languages: Languages to generate questions in.
    - language_weights: Optional weights for language selection.
    """
...
class MoveGenerator:
    ...
    def __init__(self, num_disks: int, pegs: list[int], start: int, target: int, dataset=None, language: str = "en"):
    ...
    def _move_disk(self, from_peg: int, to_peg: int):
        ...
        move_text = self.dataset._get_translation(
            "move_template", 
            self.language,
            disk=disk,
            from_peg=from_peg,
...
class HanoiDataset(MultilingualProceduralDataset):
    """
    ...
    def _generate_item(self, idx: int, language: str) -> dict[str, Any]:
        """
        Generate a Tower of Hanoi problem instance in the specified language.
        ...
        move_gen = MoveGenerator(num_disks, pegs, start_peg, target_peg, dataset=self, language=language)
        try:
        ...
        if self.visualize:
            ...
            for move in solution:
                ...
                try:
                    disk, from_peg, to_peg = self._parse_move(move, language)
                except ValueError as ve:
                    # print(f"Error parsing move: {ve}")
                    raise ve

                # Validate the move
                if not self._validate_move(pegs_state, move, language):
        ...
        # Peg labels
        peg_labels = {peg: self._get_translation("peg_label", language, peg_number=peg) for peg in pegs}
    ...
    def _validate_move(self, pegs_state: dict[int, list[int]], move: str, language: str = "en") -> bool:
        """
        Validate that a move adheres to the Tower of Hanoi rules.

        Args:
            pegs_state (dict): Current state of the pegs.
            move (str): Move instruction in the specified language.
            language (str): Language code for parsing.
    ...
    def _parse_move(self, move: str, language: str = "en") -> tuple[int, int, int]:
        """
        Parse a move string and extract disk number, from peg, and to peg.

        Args:
            move (str): Move instruction in the specified language.
            language (str): Language code for pattern selection.

        Returns:
            tuple: (disk, from_peg, to_peg)
        """
        pattern = self._get_translation("move_pattern", language)
    ...
    def score_answer(self, answer: Optional[str], entry: dict[str, Any]) -> float:
        ...
        for move in moves:
            ...
            if not self._validate_move(peg_state, move, language):
                return 0.0
