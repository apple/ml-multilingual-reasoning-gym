...
@dataclass
class TsumegoConfig(DatasetConfig):
...
class TsumegoDataset(MultilingualProceduralDataset):
    ...
    def _generate_item(self, idx: int, language: str) -> dict[str, Any]:
        ...
        Args:
            idx: The index of the problem to generate
            language: The language to use for the problem text
        ...
        # Get a random prompt template using translation
        prompt_key = f"prompt_template_{rng.randint(1, 3)}"
        question = self._get_translation(prompt_key, language, board_str=board_str)
