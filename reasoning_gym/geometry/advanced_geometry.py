...
@dataclass
class AdvancedGeometryConfig(DatasetConfig):
    """
...
class AdvancedGeometryDataset(MultilingualProceduralDataset):
    """
    ...
    def _generate_item(self, idx: int, language: str) -> dict[str, Any]:
        """
        Generate a single advanced geometry item based on the config's task types for a specific language.
        """
        ...
        if task_type == "orthocenter":
            question, answer, metadata = self._build_orthocenter_task(rng, A, B, C, language)
        elif task_type == "incircle_radius":
            question, answer, metadata = self._build_incircle_radius_task(rng, A, B, C, language)
        elif task_type == "angle_measure":
            question, answer, metadata = self._build_angle_measure_task(rng, A, B, C, language)
        else:
    ...
    def _build_orthocenter_task(self, rng: random.Random, A: Point, B: Point, C: Point, language: str):
        """
        ...
        # Choose a random template variant
        template_variant = rng.choice(["variant1", "variant2"])
        question = self._get_translation(
            f"orthocenter_{template_variant}",
            language,
            A=(A.x, A.y),
            B=(B.x, B.y),
            C=(C.x, C.y),
            format_instructions=self._get_translation("format_instructions", language)
        )
    ...
    def _build_incircle_radius_task(self, rng: random.Random, A: Point, B: Point, C: Point, language: str):
        """
        ...
        template_variant = rng.choice(["variant1", "variant2"])
        question = self._get_translation(
            f"incircle_radius_{template_variant}",
            language,
            A=(A.x, A.y),
            B=(B.x, B.y),
            C=(C.x, C.y),
    ...
    def _build_angle_measure_task(self, rng: random.Random, A: Point, B: Point, C: Point, language: str):
        """
        ...
        template_variant = rng.choice(["variant1", "variant2"])
        question = self._get_translation(
            f"angle_measure_{template_variant}",
            language,
            A=(A.x, A.y),
            B=(B.x, B.y),
            C=(C.x, C.y),
