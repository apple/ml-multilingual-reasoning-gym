...
@dataclass
class CourseScheduleConfig(DatasetConfig):
...
class CourseScheduleDataset(MultilingualProceduralDataset):
    ...
    def _generate_item(self, idx: int, language: str) -> dict[str, Any]:
        """Generate a single Course Schedule question for a specific language"""
        ...
        # Get localized true/false values for the question template
        true_answer_text = self._get_translation("true_answer", language)
        false_answer_text = self._get_translation("false_answer", language)
        
        # Use translation templates for the question
        ...
        # Localize the True/False answer
        localized_answer = self._get_translation(
            "true_answer" if answer else "false_answer",
            language
        )
