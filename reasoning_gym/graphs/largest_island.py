...
@dataclass
class LargestIslandConfig(DatasetConfig):
...
class LargestIslandDataset(MultilingualProceduralDataset):
    ...
    def _generate_item(self, idx: int, language: str) -> dict[str, Any]:
        """Generate a single Largest Island question in the specified language"""
        ...
        question = self._get_translation("question_template", language, 
                                       rows=rows, cols=cols, grid=grid_str)
