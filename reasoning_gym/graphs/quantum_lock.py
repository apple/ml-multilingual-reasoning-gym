...
@dataclass
class QuantumLockConfig(DatasetConfig):
...
class QuantumLockDataset(MultilingualProceduralDataset):
    ...
    def _generate_item(self, idx: int, language: str) -> dict[str, Any]:
        """Generate a single QuantumLock task for a given language
        ...
        return {
            "question": self.format_puzzle(language, puzzle=puzzle_data),
            "answer": self._get_translation("separator", language).join(puzzle_data["solution"]),
            "metadata": {
    ...
    def format_puzzle(self, language: str, puzzle: dict) -> str:
        # Format button descriptions
        button_descriptions = []
        separator = self._get_translation("separator", language)
        
        for btn in puzzle["buttons"]:
            operation_desc = self._get_translation(f"operation_{btn['type']}_template", language, value=btn['value'])
            state_name = self._get_translation(f"state_{btn['active_state']}", language)
            when_text = self._get_translation("when_state", language, state=state_name)
            condition = self._get_translation("condition_format", language, when_text=when_text)
            
            button_desc = self._get_translation("button_format", language,
                                              name=btn['name'],
                                              operation=operation_desc,
                                              condition=condition)
            button_descriptions.append(button_desc)
        
        buttons_text = "\n".join(button_descriptions)
        
        # Localize initial state
        initial_state_localized = self._get_translation(f"state_{puzzle['initial_state']}", language)
        
        # Use single template
        return self._get_translation("prompt_template", language,
                                   initial_value=puzzle['initial_value'],
                                   initial_state=initial_state_localized,
