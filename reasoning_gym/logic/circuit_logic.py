...
@dataclass
class CircuitLogicConfig(DatasetConfig):
    """
    ...
    :param languages: List of languages or single language to generate content in
    :param language_weights: Optional weights for language sampling when using multiple languages
...
class CircuitLogicDataset(MultilingualProceduralDataset):
    """
    ...
    def _generate_item(self, idx: int, language: str) -> dict[str, Any]:
        """
        Generate one random circuit logic item using ASCII drawing for the specified language.
        """
    ...
    def _generate_circuit(
        self, rng: Random, num_terms: int, min_inputs: int, max_inputs: int, neg_prob: float, allow_reuse: bool, language: str
    ) -> dict[str, Any]:
        """
        ...
        for inp in inputs_list:
            label = f"{inp}{colon}" + _repeat(HORIZ, input_width - (len(inp) + len(colon)))
        ...
        lines = []
        lines.append(self._get_translation("circuit_intro", language))
        lines.append(ascii_diagram)
        lines.append("")
        legend_lines = []
        legend_lines.append(self._get_translation("legend_title", language))
        for op_name, _, draw_sym in self.internal_ops:
            legend_lines.append(f"{draw_sym*2}{colon}{op_name}")
        if neg_prob > 0:
            legend_lines.append(self._get_translation("negate_symbol", language, symbol=">o"))
        if final_gate_sym not in self.internal_ops:
            legend_lines.append(f"{final_gate_sym*2}{colon}{final_gate_name}")
        legend_str = "\n".join(legend_lines)

        lines.append(legend_str)
        lines.append("")
        lines.append(self._get_translation("input_assignments", language))
        for inp in inputs_list:
            lines.append(self._get_translation("assignment_format", language, input=inp, value=assignments[inp]))
        lines.append("")
        lines.append(self._get_translation("output_question", language))
