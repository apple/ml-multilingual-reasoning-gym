...
@dataclass
class PropositionalLogicConfig(DatasetConfig):
...
class PropositionalLogicDataset(MultilingualProceduralDataset):
    ...
    def _generate_item(self, idx: int, language: str) -> dict[str, Any]:
        ...
        # Format question
        question_format = self._get_translation("question_format", language)
        question = question_format
        given_text = self._get_translation("given_text", language)
        question += given_text + "\n"
        for i, premise in enumerate(premises, 1):
            question += f"{i}. {premise}\n"
        conclusion_question = self._get_translation("conclusion_question", language)
