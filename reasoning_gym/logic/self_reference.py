...
def generate_dynamic_puzzle(difficulty, rng, language, get_translation=None):
    """
    ...
    statements_text = [
        get_translation("statement_1", language, number=a_param),
        get_translation("statement_2", language, number=b_param),
        get_translation("statement_3", language, number=c_param),
        get_translation("statement_4", language, number=d_param),
        get_translation("statement_5", language),
        get_translation("statement_6", language),
        get_translation("statement_7", language),
    ]
...
@dataclass
class SelfReferenceConfig(DatasetConfig):
...
class SelfReferenceDataset(MultilingualProceduralDataset):
    ...
    def _generate_item(self, idx: int, language: str) -> dict[str, Any]:
        """Generate a single SelfReference task for the given language
        ...
        # Generate puzzle
        puzzle = generate_dynamic_puzzle(difficulty, rng, language, self._get_translation)
        puzz_s = (
            self._get_translation("puzzle_intro", language) + "\n"
