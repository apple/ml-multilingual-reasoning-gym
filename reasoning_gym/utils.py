...
def maybe_decompose_dataset_name(name: str, registry: Optional[dict] = None) -> tuple[str, list[str]]:
    """
    Decompose dataset name into base name and languages.

    If a registry is provided and the exact name exists in it, returns the name as-is
    without decomposition (supports labeled variants like "maze:easy").
    Otherwise, decomposes the name to extract language suffixes.

    Args:
        name: Dataset name, optionally with language suffix (e.g., "basic_arithmetic:en,de")
        registry: Optional dataset registry to check for exact matches

    Returns:
        Tuple of (base_name, languages_list)

    Examples:
        "basic_arithmetic" -> ("basic_arithmetic", [])
        "basic_arithmetic:en" -> ("basic_arithmetic", ["en"])
        "basic_arithmetic:en,de,ja" -> ("basic_arithmetic", ["en", "de", "ja"])
        "maze:easy" with "maze:easy" in registry -> ("maze:easy", [])
    """
    # If exact name is in registry, don't decompose (supports labeled variants)
    ...
    base_name, language_spec = name.split(":", 1)
    languages = [lang.strip() for lang in language_spec.split(",")]
    return base_name, languages
